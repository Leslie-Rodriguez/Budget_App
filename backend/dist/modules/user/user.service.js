...
let UserService = class UserService {
    ...
    async update(user_id, updateUserDto) {
        const user = await this.usersRepository.preload(Object.assign({ user_id: user_id }, updateUserDto));
        if (!user) {
            throw new common_1.NotFoundException(`Item ${user_id} not found`);
        }
        return this.usersRepository.save(user);
    }
};
